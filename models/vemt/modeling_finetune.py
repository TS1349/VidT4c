...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, return_tokens=False):
        ...
        if return_tokens:
            global_f = self.fc_norm(x.mean(1)) if self.fc_norm is not None else self.norm(x[:, 0])
    ...
    def forward_features_until(self, x, end_block_idx):
        """Forward up to (excluding) blocks[end_block_idx]. Mirrors AdaMAE helper.
    ...
    def forward_features_from(self, h, start_block_idx):
        ...
        Mirrors AdaMAE helper. Returns the global feature [B, D].
        """
    ...
    def forward(self, x, return_feat=False, return_tokens=False):
        result = self.forward_features(x, return_tokens=return_tokens)

        if return_tokens:
            global_f, tokens = result
            if self.args.set_video_only:
                out = self.head_dropout(global_f)
                out = self.head(out)
                if self.args.dataset in ('emognition', 'mdmer'):
                    x_v = out[:, :self.num_value].unsqueeze(-1)
                    x_a = out[:, self.num_value:].unsqueeze(-1)
                    out = torch.concat((x_v, x_a), dim=-1)
                if return_feat:
        ...
        video_f = result  # [B, D]
        if self.args.set_video_only:
            out = self.head_dropout(video_f)
            out = self.head(out)
            # NOTE: skip the V/A reshape here when multi-clip (K-clip / dense N-clip)
            # — caller pools first then reshapes. Mirrors AdaMAE behavior.
        ...
        # No head (eeg_signal path) — return video_f as logit placeholder.
        if return_feat:
