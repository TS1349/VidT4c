...
def normalize_ch_name(ch_name: str) -> str:
    return ch_name.strip().upper()


def get_input_chans_from_names(ch_names, standard_1020=STANDARD_1020):
    standard_map = {ch: i for i, ch in enumerate(standard_1020)}

    input_chans = [0]  # cls token
    for ch_name in ch_names:
        ch_name = normalize_ch_name(ch_name)
        if ch_name not in standard_map:
            raise ValueError(
                f"Channel '{ch_name}' not found in STANDARD_1020.\n"
                f"Available dataset channels must be a subset of STANDARD_1020."
            )
        input_chans.append(standard_map[ch_name] + 1)
...
class LaBraM_Model(nn.Module):
    def __init__(self, args, EEG_size=2000, patch_size=200, in_chans=1, out_chans=8, output_dim=1000, embed_dim=200, depth=12,
        ...
        super(LaBraM_Model, self).__init__()
        ...
        self.in_chans = 1 # Fix as 1 following LaBraM
        self.eeg_ch = in_chans # Reduce channel to 1
        ...
        self.patch_embed = TemporalConv(out_chans=out_chans) if self.in_chans == 1 else PatchEmbed(EEG_size=EEG_size, patch_size=patch_size, in_chans=self.in_chans, embed_dim=embed_dim)
        ...
        # Mapping input_chans to fit LaBraM repo
        if self.args.dataset.lower() not in DATASET_CH_NAMES:
            raise ValueError(
                f"Unsupported dataset: {self.args.dataset}. "
                f"Supported: {list(DATASET_CH_NAMES.keys())}"
            )

        self.dataset_ch_names = [
            normalize_ch_name(ch) for ch in DATASET_CH_NAMES[self.args.dataset.lower()]
        ]

        default_input_chans = get_input_chans_from_names(self.dataset_ch_names, STANDARD_1020)
    ...
    def forward(self, sample, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        '''
        ...
        elif isinstance(input_chans, list):
            input_chans = torch.tensor(input_chans, dtype=torch.long, device=eeg.device)
        else:
            input_chans = input_chans.to(eeg.device)

        x = self.forward_features(eeg, input_chans=input_chans, return_patch_tokens=return_patch_tokens, return_all_tokens=return_all_tokens, **kwargs)
...
# @register_model
# def labram_base_patch200_200(pretrained=False, **kwargs): # Use base model as benchmark
