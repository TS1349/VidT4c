...
class ListPatchEmbedding(nn.Module):
    def __init__(
        ...
        if isinstance(augmentations, str):
            aug_names = [a.strip() for a in augmentations.split(",") if a.strip()]
        else:
            aug_names = list(augmentations)

        self.augmentation = nn.ModuleList([get_augmentation(a) for a in aug_names])
