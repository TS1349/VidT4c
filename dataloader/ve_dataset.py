...
def dense_video_collate_fn(batch):
    """Collate function for dense-video mode (variable N clips per sample).

    Each batch item has 'video' of shape [N_i, T, C, H, W] with N_i varying.
    Pads each sample's video to batch-max N via cycle-repeat (last clip
    repeated as needed) so the batch stacks into [B, N_max, T, C, H, W].
    Adds 'video_lengths' [B] tensor of actual N_i for mask-pooling on the
    model side. Other fields collate via default_collate.

    Falls back to default_collate when 'video_length' is absent.
    """
    if 'video_length' not in batch[0]:
        return default_collate(batch)

    videos = [item['video'] for item in batch]
    lengths = torch.stack([item['video_length'].long() for item in batch])  # [B]
    ...
    for v in videos:
        N_i = v.size(0)
        if N_i < N_max:
            n_repeat = (N_max + N_i - 1) // N_i
            v_pad = v.repeat((n_repeat,) + (1,) * (v.dim() - 1))[:N_max]
        else:
            v_pad = v[:N_max]
        padded.append(v_pad)
    video_batch = torch.stack(padded)  # [B, N_max, T, C, H, W]

    out = {'video': video_batch, 'video_lengths': lengths}
    # Collate everything else via default_collate, skipping the per-sample
    # variable-length fields we just handled.
    rest = [{k: v for k, v in item.items() if k not in ('video', 'video_length')}
            for item in batch]
    rest_collated = default_collate(rest)
...
class VERandomDataset(Dataset):
    ...
    def _get_dense_clip_idxs(self, total_frames, fi):
        ...
        Each clip = num_out_frames frames at stride `fi` → span = num_out_frames * fi.
        Clip starts are placed `stride` frames apart where:
            stride = round(span * (1 - clip_overlap_ratio))
        With clip_overlap_ratio=0 (default), stride = span → non-overlapping
        (original behavior). With clip_overlap_ratio=0.5, stride = span/2
        → 50% overlap, ~2x more clips.

        N = max(1, ceil((total_frames - span) / stride) + 1) so the last clip
        end reaches total_frames. Out-of-range indices are clamped to the last
        frame so every clip yields a full num_out_frames tensor. Covers the
        entire video.
        """
        span = self.num_out_frames * fi
        if total_frames <= 0:
            return [[0] * self.num_out_frames]

        # Clamp overlap to [0, 0.99] to keep stride >= 1.
        overlap = max(0.0, min(0.99, float(getattr(self, 'clip_overlap_ratio', 0.0))))
        if overlap > 0.0:
        ...
        if total_frames <= span:
            ...
            N = (total_frames - span + stride - 1) // stride + 1
        N = max(1, N)

        clips = []
        for n in range(N):
            start = n * stride
            idxs = [min(total_frames - 1, start + i * fi) for i in range(self.num_out_frames)]
            clips.append(idxs)
        return clips
    ...
    def __getitem__(self, idx):
        ...
        if self.dense_video_clips:
            ...
            for video_idxs in all_idxs:
                ...
                videos.append(v)
            video = torch.stack(videos)        # [N, T, C, H, W]

            # EEG: full-signal window (4D) — same logic as eeg_full_signal=True path
