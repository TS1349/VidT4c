...
def run(
    ...
    if args.model =='hicmae':
        ...
        # MAE
        # ViViT loads pretrained weights internally via HuggingFace from_pretrained;
        # skip the load_flexible path that targets AdaMAE/VideoMAE checkpoints.
        if args.pretrained and not args.set_eeg_only and getattr(args, 'vemt_video', 'AdaMAE') != 'ViViT':
            try:
        ...
        if args.pretrained and not args.set_video_only:
            ...
            # CbraMod (EEG Foundation Model)
            pretrained_path = os.path.join(os.getcwd(), 'pretrained', 'cbramod.pth')  # CBraMod
            print(f'Loading pretrained weights from {pretrained_path}')
            checkpoint = torch.load(pretrained_path, map_location='cpu')

            # map_location = torch.device(f'cuda:{param.cuda}')
            _load_state_dict(model.eeg_model, checkpoint)  # Backbone is defined as the model.model
        ...
        # K-clip auto-freeze fires only when training itself sees K-clip batches.
        # With --train_random_crop, training is K=1 so we keep the backbone fully
        # trainable; --num_clips only applies at val/test for multi-view ensemble.
        if _kclip_active and not _no_kclip_freeze and not _train_random_crop_on:
            ...
            if rank == 0:
                ...
                print(
                    f"[K-clip] num_clips={args.num_clips}: backbones frozen — "
                    f"{_v_msg}; {_e_msg}."
                )
...
if "__main__" == __name__:
    ...
    parser.add_argument('--vemt_video', type=str, default='AdaMAE',
                        choices=('VideoMAE', 'AdaMAE', 'ViViT'), help='Choose FFT transformation method')
