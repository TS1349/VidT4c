...
원래 `full length coverage` 로 돌리던 거랑 비교해서 데이터 로더 / freeze 정책 / GCN 노드 구성 새로 정리한 브랜치. 무엇이 바뀌었고 왜 그렇게 했는지 정리해두었습니다.
...
`--gcn_video_per_clip --gcn_eeg_per_clip` 옵션. K개 video / K개 EEG 가 각각 별도 노드로 graph 에 들어감 (총 2K + region 노드).
...
- `video[i] ↔ EEG[i]` : paired (같은 시간 윈도우 prior, **strict time-align**)
...
아직 구현 전. Strict 버전 결과 보고 어느 정도 prior 가 적당한지 판단할 예정. 두 방향 다 확정된 건 아니고 실험 중인 단계.
