#!/usr/bin/env bash
# MDMER fold1 — Swin (fi=2, mean → attn) on GPU 4,5,6,7
# Same lr/wd as the AdaMAE fold1 runs (1e-4 / 0.05, patience 15).
# Swin uses --dense_chunk_size 3 (hierarchical 3D is heavier per clip).
# mean and attn share the cache; cleared after both finish.
#
# Run from /SSD4/jh/INRIA. Run in parallel with run_mdmer_fold1_vivit.sh
# on GPUs 0,1,2,3.
...
run "swin_fi2_mean" \
  ...
  --dense_video_clips --dense_chunk_size 3 --dense_cache_features \
  --video_unfreeze_last_n_blocks $UNFREEZE --clip_pool mean \
  --checkpoint_dir ./checkpoints_clip \
  --experiment_name dense_fi2_1block-mean_swin_mdmer_fold1_${LR}_${WD} \
  --port 20130
...
run "swin_fi2_attn" \
  ...
  --checkpoint_dir ./checkpoints_clip \
  --experiment_name dense_fi2_1block-attn_swin_mdmer_fold1_${LR}_${WD} \
  --port 20131

clear_cache

echo "[$(ts)] >>> Swin mdmer fold1 chain DONE"
